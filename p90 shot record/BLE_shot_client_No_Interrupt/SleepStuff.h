...
void sleepLight(long last_time, uint32_t count) {  //reference back to update values
  if (millis() > (last_time + (SLEEPTIME * mS_TO_S_FACTOR)) && ENABLED)                      //if last time, in seconds, is greater then 2 minutes then sleep. Aka, if idle for 0 minutes then sleep. Deepsleep has too slow a startup to use.
  {
