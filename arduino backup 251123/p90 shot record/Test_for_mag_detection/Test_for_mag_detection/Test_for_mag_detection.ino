...
unsigned int key = 105;  //69 in hex. Read outs can be converted. Comparing in hex is a pain
const int sector = 4;
const int reloadOut = 7;
...
void validateSector() {
  int counter = 0;
  for (int i = 0; i < sizeof(str); i++) {
    if (str[i] == key)  //compare value to key and ensure it comparing it in hex (will compare memory values otherwise.
    { //compare not as hex as it makes more sense to convert data instead of converting both in different ways to match.
      Serial.print("Good");
  ...
  if (counter == sizeof(str))  //ensure that all good are equal to the block size
  {
    ...
    digitalWrite(reloadOut, HIGH);
  } //Send signal when successfully read a card
}


void blankStr() {
  for (int i = 0; i <= sizeof(str); i++)  //empties contents of str
  {
    str[i] = 15, HEX;
  }
}
