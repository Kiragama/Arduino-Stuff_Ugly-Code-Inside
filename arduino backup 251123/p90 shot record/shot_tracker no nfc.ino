...
void loop() //BUG currently when on first loop after boot, it just resets number back to 50 at a random point but never again
{
  bool reload;
  int tmp = 0;
  int ammo = mag; //ammo seperate to i because for loop had issues if I didn't
  printAmount(ammo);
  
  
  for (int i = mag; i>0;)//process for magazine counter
  {
    
    if (ammo == i) //checks if they are in sync and ticks both up
    {
      ammo--;
