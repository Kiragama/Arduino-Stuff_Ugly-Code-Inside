...
#if ENABLE_MEDIAN_FILTER //see Advancedconfig.h
...
#if ((INTERFILTER_MODE != INTERFILTER_NONE) && (FLEXION_MIXING == MIXING_SINCOS)) //Since it references Median, it seems to sample inputs and set a median for each finger. Possible basic calibration
...
int analogReadDeadzone(int pin){
  ...
  if (abs(ANALOG_MAX/2 - raw) < JOYSTICK_DEADZONE * ANALOG_MAX / 100)
    return ANALOG_MAX/2;
  else if (raw > ANALOG_MAX/2)//amplification of values
    return raw+JOYAMP;
  else
    return raw-JOYAMP;
}
...
int getJoyX(){
  ...
  return ANALOG_MAX/2;
  #elif JOY_FLIP_X
  return ANALOG_MAX - analogReadDeadzone(PIN_JOY_X)+X_SHIFTRIGHT;
  #else
  return analogReadDeadzone(PIN_JOY_X)+X_SHIFTRIGHT;
  #endif
}
...
int getJoyY(){
  ...
  return ANALOG_MAX/2;
  #elif JOY_FLIP_Y
  return ANALOG_MAX - analogReadDeadzone(PIN_JOY_Y)+Y_SHIFTUP;
  #else
  return analogReadDeadzone(PIN_JOY_Y)+Y_SHIFTUP;
  #endif
}
