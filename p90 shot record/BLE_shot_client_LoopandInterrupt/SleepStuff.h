...
void setSleepwakeup() {
  ...
  //esp_sleep_enable_ext0_wakeup((gpio_num_t)PIN, GPIO_INTR_HIGH_LEVEL);
  esp_sleep_enable_gpio_wakeup();
}

void sleepLight(bool overide) {  //reference back to update values
  if (SLEEPENABLED && millis() > (last_time + (SLEEPTIME * mS_TO_S_FACTOR)) || overide)  { //if last time, in seconds, is greater then 2 minutes then sleep. Aka, if idle for 0 minutes then sleep. Deepsleep has too slow a startup to use.
    if(USING_INTERRUPT){
