...
void IRAM_ATTR isr() {  //esp32 specific funtion for interrupts. arduino has set pins while esp can do any. Cannot be any output during interrupt and this includes Serial output
  ...
  if (micros() > (last_time + 10000))  //wait 100000 microseconds between shots
  {
    count++;  //count increments so if multiple shots trigger before upload, it is still counted
    //last_time = micros();
    //Serial.println("count as of shot read ");
    //Serial.print(count);
    //Serial.println();
  }
}  //interrupts better for project as it gets around the issue with waiting for the cycle to reach the check command.
...
*/
  ...
  last_time = micros();  //Initial setup of the time
  ...
  //_______________________________________________________________________________ End of interrupt for testing
...
void loop() {
  ...
  /*if (micros() > (last_time + 300000000))  //if last time, in seconds, is greater then 5 minutes then sleep. Aka, if idle for 5 minutes then sleep
  {
  ...
  if (connected) {
    ...
    if (count) {
      String value = String(count);                                      //convert https://circuits4you.com/2018/03/09/how-to-convert-int-to-string-on-arduino/                                                   //not 0 and to not send needless data
