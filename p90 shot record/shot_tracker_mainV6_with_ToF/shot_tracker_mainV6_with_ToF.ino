...
const int8_t BAT_X = 3;
const float minBat = 6.7;
const int8_t BAT_WIDTH = 4;                                  //width of internal volume
...
void drainBat() {
  ...
  lcd_1.fillRect(BAT_X, BAT_Y + BAT_WOFFSET - 1, BAT_WIDTH, final_val, SSD1306_INVERSE);  //creates a box at the top of bat and this is made of inverse colour. This gets bigger as it drains and grows downwards. should end at BAT_HEIGHT
...
void loop() {  //loop must always be cycling
  ...
  if (box.ammo > 0)  //process for magazine counter. Simple process to loop, count and stop when mag capacity reached
  {
    ...
    //delay(10);  //delay after shot to avoid double fires from the light still being on
  }
  drainBat();
  box.refreshCounter++;
  ...
  if (box.ammo == 0)  //when ammo 0, starts reload process
  {
    ...
    if (digitalRead(chkReload) == HIGH)  //stops everything until reloaded.
    {
      box.ammo = box.magazine;
      delay(200);
    }
