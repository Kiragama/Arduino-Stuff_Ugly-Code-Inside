...
void Display_Stuff::printText(String text, int size) {
  setFont(&FreeSans12pt7b);
  fillScreen(BACKCOLOUR);
  setCursor(10, -30);
  setTextSize(size);
...
void Display_Stuff::printAmount() {  //print current mag count
  ...
  if (getammo() <= 0) {
    ...
    setTextSize(NUMSIZE);
    loadTextNum(NUM_X, NUM_Y, getlastammo(), getammo());  //prints text and overwrites last text
    setlastammo(getammo());
  }
}
...
int16_t Display_Stuff::setAmmo() {  //sets ammo
  ...
  while (clrBtn)  //this is so if the clear button pressed, if not cleared then this should never loop
  {
    ...
    setCursor(x, height() - 25);  //50 from the bottom
    drawFastHLine(x, height() - 20, width() - 70, TEXTCOLOUR);
  ...
  while (breakL) {
    ...
    switch (getkeyPressed()) {
      ...
      case 1:
        ...
        menee.saveEE(getMag(), EEADDRESS);  //saves to EEPROM
        break;
      case 2:
        //save
        menee.saveEE(getMag(), EEADDRESS);
        break;
      case 3:
        //load
        setMag(menee.loadEE(EEADDRESS));
        setammo(getMag());
        break;
      case 12:
